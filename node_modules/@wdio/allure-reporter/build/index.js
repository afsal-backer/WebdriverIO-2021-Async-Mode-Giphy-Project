...
class AllureReporter extends reporter_1.default {
    ...
    onSuiteEnd(suite) {
        if (this._options.useCucumberStepReporter && suite.type === 'scenario') {
            ...
            const isSkipped = suite.tests.every(item => [constants_1.SKIPPED].indexOf(item.state) >= 0) && suite.hooks.every(item => [constants_1.PASSED, constants_1.SKIPPED].indexOf(item.state) >= 0);
            if (isSkipped) {
            ...
            const isPartiallySkipped = suiteChildren.every(item => [constants_1.PASSED, constants_1.SKIPPED].indexOf(item.state) >= 0);
